            ...
            with tags.table(
                ...
                with tags.thead():
                    ...
                    if settings.TYPE == "ASSESS":
                        ...
                        tr += tags.th("Train Metric (on %s)" % settings.YTRAIN_LABEL)
                    ...
                    if settings.TYPE == "ASSESS":
                        tr += tags.td(
                            bootstrap_radio_group(
                                generate_buttons(tables, "metricname")
                            )
                        )
                    elif settings.TYPE == "CV_TT":
                        tr += tags.td(
                            bootstrap_radio_group(
                                generate_buttons(tables, "trainmetricname")
                            )
                        )
                        tr += tags.td(
                            bootstrap_radio_group(
                                generate_buttons(tables, "testmetricname")
                            )
                        )
                    tr += tags.td(
