    ...
    for key in keys:
        ...
        if key == 'time':
            ...
            if b_func == min:
                ...
                raise AbedNonstandardMetricDirection
        mean_ranks = [0]*len(settings.METHODS)
        for i, dset in enumerate(sorted(settings.DATASETS)):
            dsetlength = max(dsetlength, len(dset))
            row = [splitext(basename(dset))[0]]
            ranks = get_ranks(list([round(x, 4) for x in tables[key][i, ]]))
            if b_func == max:
                ranks = [len(ranks) - t + 1 for t in ranks]
            row += ['%f' % x for x in ranks]
            table.append(row)
            for i, r in enumerate(ranks):
                mean_ranks[i] += float(r)/float(len(settings.DATASETS))
        dashrow = ['-'*dsetlength] + ['-'*len(x) for x in 
                sorted(settings.METHODS)]
        table.append(dashrow)
        table.append(['Mean ranks'] + ['%.2f' % x for x in mean_ranks])
        import code
        code.interact(local=dict(globals(), **locals()))

        opath = '{respath}{sep}ABED_{metric}_{key}_ranks.txt'.format(
                respath=settings.OUTPUT_DIR, sep=os.sep, metric=metricname,
                key=key)
        now = datetime.datetime.now()
        headers = [''] + sorted(settings.METHODS)
